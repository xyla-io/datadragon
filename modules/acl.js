...
var Acl = function () {
  ...
  this.aclInstance.addUserRoles(roles.user, roles.user);
  this.aclInstance.addUserRoles(roles.guest, roles.guest);
};
...
Acl.prototype.middleware = function(numberOfPathComponents, denyMiddleWare) {
  return (req, res, next) => {
    let roles = (req.isAuthenticated()) ? [this.roles.user] : [this.roles.guest];
